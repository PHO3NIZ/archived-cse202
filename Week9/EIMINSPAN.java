...
public class EIMINSPAN {
    ...
    private static int prim(Vertex[] vertices) {
        PriorityQueue<Edge> queue = new PriorityQueue<>((v1, v2) -> Integer.compare(v1.weight, v2.weight));
        queue.add(new Edge(vertices[0], 0));

        int count = 0;
        int total = 0;
        while (!queue.isEmpty()) {
            Edge edge = queue.remove();
            if (edge.endPoint.isDiscover)
                continue;
            count++;
            total += edge.weight;
            edge.endPoint.isDiscover = true;

            for (Edge e : edge.endPoint.edges) {
                if (!e.endPoint.isDiscover) {
                    queue.add(e);
                }
            }
        }
