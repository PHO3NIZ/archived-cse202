...
public class EILOCALA {
    public static void main(String[] args) {
        ...
        dfs(vertices[0]);
        int temp = maxVertexId;
        for (int i = 0; i < n; i++) {
            vertices[i].isDiscover = false;
            vertices[i].lengthFromRoot = 0;
        }
        maxLength = -1;
        maxVertexId = -1;
        dfs(vertices[temp]);
        System.out.println(Math.min(temp, maxVertexId) + " " + maxLength);
    }

    static int maxLength = -1;
    static int maxVertexId = -1;

    public static void dfs(Vertex v) {
        v.isDiscover = true;
        if (v.lengthFromRoot > maxLength) {
            maxLength = v.lengthFromRoot;
            maxVertexId = v.id;
        } else if (v.lengthFromRoot == maxLength && v.id < maxVertexId) {
            maxVertexId = v.id;
        }
        for (Edge e : v.edges) {
